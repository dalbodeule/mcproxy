...
func (_q *AuditQuery) QueryServer() *ServerQuery {
...
func (_q *AuditQuery) WithServer(opts ...func(*ServerQuery)) *AuditQuery {
	query := (&ServerClient{config: _q.config}).Query()
...
func (_q *AuditQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Audit, error) {
	...
	_spec.Node.Schema = _q.schemaConfig.Audit
	...
	if query := _q.withServer; query != nil {
		if err := _q.loadServer(ctx, query, nodes, nil,
			func(n *Audit, e *Server) { n.Edges.Server = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AuditQuery) loadServer(ctx context.Context, query *ServerQuery, nodes []*Audit, init func(*Audit), assign func(*Audit, *Server)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Audit)
	for i := range nodes {
		if nodes[i].server_audits == nil {
			continue
		}
		fk := *nodes[i].server_audits
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "server_audits" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *AuditQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(_q.schemaConfig.Audit)
	ctx = internal.NewSchemaConfigContext(ctx, _q.schemaConfig)
