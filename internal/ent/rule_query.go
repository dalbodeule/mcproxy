...
func (_q *RuleQuery) QueryServer() *ServerQuery {
...
func (_q *RuleQuery) WithServer(opts ...func(*ServerQuery)) *RuleQuery {
	query := (&ServerClient{config: _q.config}).Query()
...
func (_q *RuleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Rule, error) {
	...
	_spec.Node.Schema = _q.schemaConfig.Rule
	...
	if query := _q.withServer; query != nil {
		if err := _q.loadServer(ctx, query, nodes, nil,
			func(n *Rule, e *Server) { n.Edges.Server = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RuleQuery) loadServer(ctx context.Context, query *ServerQuery, nodes []*Rule, init func(*Rule), assign func(*Rule, *Server)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Rule)
	for i := range nodes {
		if nodes[i].server_rules == nil {
			continue
		}
		fk := *nodes[i].server_rules
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "server_rules" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *RuleQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(_q.schemaConfig.Rule)
	ctx = internal.NewSchemaConfigContext(ctx, _q.schemaConfig)
